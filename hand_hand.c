...
void StatusDeal(uint8_t message) //message=0表示无串口信息,否则有串口信息
{
    ...
    {
        case 0:     //正常运行状态
            if(message == 1 && keep == 0 && time_count==0) //识别到停止标识,树莓派发送停止指令后自动待机
            {
                if(cross>0) //经过路口
                {
                    cross--;
        ...
        case 1:     //正在停止状态,需要检查是否已经停下,停止不使用闭环
            ...
            if(stop_judge <=0)
            {
                ...
                /*if(route_flag == 1 && cross == 1) //有药品,即送药过程,且不为第一次停止
                {
                ...
                if(question==1)
                {
                    if(route_flag != 0 && yellow_light == 1) //母车有转向信息,且自选点未完成
                    {
                        status_hand = 3;
                        turn_route_flag = 3 - route_flag; //子车反向转

                    }
                    else if(yellow_light == 0) //自选点完成,且母车已释放阻塞信息
                    {
                        status_hand = 3;
                        turn_route_flag = 0;
                    }
            ...
            if(stop_judge <= 0)
            {
                ...
                switch(turn_route_flag) //提取闭环指示
                {
                    ...
                    case 3: //掉头,仅在到达药房时执行这一分支
                        ...
                        {
                            if(question == 1 && yellow_light == 1)
                            {
                                LED_flag = 2;
                                if(message == 3 && strcmp((char*)rData1,"X ")) //母车完成卸药
                                {
                                    LED_flag = 0;
                                }
                                else if(message == 3 && strcmp((char*)rData1,"ok ")) //母车释放阻塞
                                {
                                    yellow_light = 0; //自选点任务完成
                                    task_flag = 1;
                                }
                            }
                            else if(question == 1 && yellow_light == 0)
                            {
                                task_flag = 1;
                            }
                            else if(question == 2)
                ...
                if(task_flag == 1) //完成闭环任务
                {
                    ...
                    if(question == 1 && yellow_light == 1) //尚未完成自选点
                    {
                        status_hand = 7; //修改为正常运行状态
                    }
                    else if(question == 1 && yellow_light == 0) //完成自选点
                    {
                        status_hand = 8;
                    }
        ...
        case 4:     //指令等待状态/初始状态
            if((question == 1 && Drug_Read()==1)||
                (question == 2 && message == 3 && strcmp((char*)rData1,"X ") == 0)) //第一问 或 子车启动信号
            {
            ...
            else if(question == 2 && route_flag !=0) //母车有传输转弯数据
            {
